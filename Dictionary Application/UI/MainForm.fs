    ...
    override this.OnLoad(e) =
        ...
        this.BtnRegister.Click.Add(fun _ -> 
            if String.IsNullOrWhiteSpace(txtUser.Text) || String.IsNullOrWhiteSpace(txtPass.Text) then
                ...
                // 1. تحويل النص المختار إلى نوع UserRole (Enum)
                let selectedRole = 
                    if cmbRole.Text = "Admin" then UserRole.Admin else UserRole.NormalUser
                
                // 2. إرسال النوع الصحيح للدالة
    ...
    do
        this.Controls.AddRange(
            [| lblTerm; txtTerm; lblDef; txtDef;
               btnAdd; btnUpdate; btnDelete; btnPrintAll; btnLogout;
    ...
    // EVENTS =========================================
        ...
        btnAdd.Click.Add(fun _ ->
            ...
            if not (obj.ReferenceEquals(newDict, dict)) then
                ...
                saveDict()
                clearInputs()
                refreshList() // Refresh automatically
        )
        ...
        btnSearchExact.Click.Add(fun _ ->
            ...
            | None ->
                displayResults []
                MessageBox.Show("No exact match.") |> ignore
        )

        // PRINT ALL
        btnPrintAll.Click.Add(fun _ ->
            refreshList()
        )
...
type LoginForm() as this =
    ...
    do
        this.Controls.AddRange([| lblUser; txtUser; lblPass; txtPass; btnLogin; btnGoToReg |])
    ...
    override this.OnLoad(e) =
        ...
        btnLogin.Click.Add(fun _ ->
            ...
            | Ok user ->
                ...
                use mainForm = new MainForm(user) // نفتح الأساسي
                mainForm.ShowDialog() |> ignore
                this.Show() // لما الأساسي يتقفل، نظهر اللوجين تاني
                txtPass.Text <- "" // نفضي الباسورد
            | Error msg ->
...
let main argv =
    ...
    Application.Run(new LoginForm()) // نبدأ باللوجين
    0